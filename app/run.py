...
def configure_logging(log_level="INFO"):
    ...
    # Handler para arquivo de log global (append) - mantém funcionalidade atual
    logs_dir = ROOT_DIR / "logs"
    logs_dir.mkdir(exist_ok=True)
    global_log_path = logs_dir / "global.log"
    logger.add(global_log_path, level="INFO", format=log_format, rotation="10 MB", compression="zip", enqueue=True, encoding='utf-8')

    # Handler para arquivo de log detalhado da execução atual (mantém compatibilidade)
    timestamp = datetime.now().strftime("%Y_%m_%d_%H%M%S")
    run_log_path = logs_dir / f"{timestamp}.log"
    logger.add(run_log_path, level="DEBUG", format=detailed_log_format, enqueue=True, encoding='utf-8')
